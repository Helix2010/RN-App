...
export function TokenDetailScreen({
  ...
  return (
    <CollapsingHeader
      ...
      expanded={
        <Stack gap="$1">
          <Heading fontSize={26}>{data?.token.symbol ?? ""}</Heading>
          {data ? (
            <Body>{`${data.token.name} · ${chainName(chain)}`}</Body>
          ) : null}
        </Stack>
      }
      collapsed={
        data ? (
          <Stack flex={1}>
            ...
            <Row alignItems="center" gap="$2" testID="token-compact-price">
              <TokenPrice price={data.priceUsd} locale={locale} size={13} />
        ...
        <>
          <Stack gap="$1">
            <Row alignItems="baseline" gap="$3">
              <TokenPrice price={data.priceUsd} locale={locale} size={30} big />
              <PriceChange value={data.change24hPct} />
            </Row>
            <Body fontSize={12}>
          ...
          {security ? (
            ...
            >
              <Row justifyContent="space-between" alignItems="center">
                <SectionTitle fontSize={14}>{t("dex.security")}</SectionTitle>
                <InlineText
                  fontSize={12}
                  fontWeight="800"
              ...
              <Row flexWrap="wrap" gap="$2">
                ...
                />
                <Check
                  ok={security.buyTaxBps <= 500 && security.sellTaxBps <= 500}
              ...
              {(trades.data ?? []).map((trade) => (
                ...
                >
                  ...
                  <InlineText
                    ...
                    width={50}
                    color={
                      trade.side === "buy" ? "$pricePositive" : "$priceNegative"
                    }
                  >
                    {trade.side === "buy"
                      ? t("dex.trades.buy")
