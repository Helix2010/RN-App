...
// AppStoreInfo.plist（generateAppStoreInformation）放在 .ipa 旁边、同名换后缀：Windows / Linux
// 上的 iTMSTransporter 上传要带它。Xcode 没生成就只记一行，不当失败——Mac 上传用不到它
const appStoreInfo = resolve(exportDirectory, "AppStoreInfo.plist");
if (existsSync(appStoreInfo)) {
  const appStoreInfoArtifact = artifact.replace(
    /\.ipa$/,
    ".AppStoreInfo.plist",
  );
  copyFileSync(appStoreInfo, appStoreInfoArtifact);
  console.log(
    `AppStoreInfo.plist: ${appStoreInfoArtifact} (${statSync(appStoreInfo).size} bytes)`,
  );
} else {
  console.log("AppStoreInfo.plist was not generated by this Xcode");
}
...
if (!flag("--upload")) {
  // 打包机上这一句不代表"没传"：那边由上传账户另外传（或者把 .ipa 交回平台），不走这个脚本
  console.log(
    "本脚本没有上传（打包机由平台另行交付；手工发版加 --upload 才传 App Store Connect，传完记下过期日：每个 TestFlight build 90 天后失效）。",
  );
  process.exit(0);
}
