...
3. **层级只在有内容时出现**：二级行只在当前一级有子标签时渲染；"更多 ▾"只在轮播标签超过行内上限时出现（§10）。
...
- 内容：`全部` + 前 8 个轮播标签（横滑）+ `更多 ▾`（钉在行右侧，不随内容横滑；见 §10）。chip 高 34、圆角 999、字 13/700；选中 `$color` 底 `$background` 字；未选 `$surfaceVariant` 底。
- 不在行内的一级（"更多"面板、搜索结果或深链带来的）以选中态插到行尾（与行内标签并存），保证"我选的"始终可见；切回其它一级时移除。
- 标签名用 `labelTranslation`（轮播标签全有），中文环境显示中文。
...
- `PickerSheet`，标题"全部分类"，列出**全部轮播标签**并保持平台 `carousel_sort` 顺序（不分组；超过 20 个才出现面板内搜索），当前一级带选中勾；选中即关闭并应用。
- 不再拉标签全集：第一版用 `/tags?limit=500` 的 188 个标签里 175 个不在轮播、绝大多数没有活跃事件且混有 `recurring`、`hide-from-new` 等运营标签，与网页版不一致（§10）。
...
| 分页失败 | 已有脚注"加载失败 · 重试" |
| 轮播标签不超过 9 个 | 全部内联，"更多 ▾" 不渲染（溢出只有 1 个不值得一个面板） |
...
**保留的取舍**：收藏视图是逐个查询的集合，搜索在集合内本地过滤，不走服务端；列表计数只在翻完（没有下一页）时显示，不显示估数。（"更多 ▾" 的出现条件与数据源已在 §10 修订；同目录的 `.html` 是首版设计稿的独立页面，未随修订更新。）
...
## 10. 修订（2026-09-10）：一级"更多"改为轮播溢出，预测市场移除 Mock 依赖

### 10.1 为什么改

- 第一版"更多"来自标签全集（188 个）：网页版 `TopNav` 只渲染轮播标签，全集仅用于内部查找，两端不一致；抽样 20 个非轮播标签只有 4 个有活跃事件，且混有 `recurring`、`hide-from-new` 等运营标签。
- 13 个轮播标签全部内联时行长约 3.3 屏（中文）到 4.7 屏（英文），排在末尾的"更多"要滑三四次才能看到。

### 10.2 现在的行为

| 项 | 规则 |
| --- | --- |
| 数据源 | 一级与"更多"同一份：`GET /tags?is_carousel=true&order=carousel_sort&ascending=true`；二级不变：`/tags/{id}/related-tags/tags` |
| 行内上限 | `PRIMARY_INLINE_LIMIT = 8`（`model/filter-state.ts`）：8 个 ≈ 375 宽两屏，"更多"一次横滑内可见；落在 Baymard 5–10 / Miller 7±2 区间上限 |
| 溢出规则 | `splitPrimaryTags`：总数 ≤ 上限 + 1 全部内联、不出"更多"；否则前 8 个内联、其余进面板，顺序均为 carousel_sort |
| "更多"位置 | 钉在一级行右侧、不随内容横滑（页头与滚走后的悬浮条同一份元素） |
| 面板 | 全部轮播标签，不分组、保持运营顺序，>20 个才出现搜索框；当前一级带勾 |
| 选中溢出项 | 以选中态钉在"更多"左侧（行外、不随横滑，最宽 168 后省略号）；切回其它一级时移除。模拟器核对时发现插在行尾会被横滑遮住，故改为钉住 |
| 非轮播一级 | 搜索结果里选的标签对象随选择记下；深链带来的用 `getTag(id)`（`/tags/{id}`）解析名字，不再拉全集。当前没有调用方传深链 `initialFilters`，这条路径为深链预留 |

### 10.3 预测市场"禁止 Mock"

预测行情、账户、结算、争议本就接真实平台（`gateway-context.tsx` 只装配 `HttpPredictGateway` / `HttpPredictAccountGateway`），本次清掉残留的 Mock 耦合并加护栏：

- 预测界面与首页预测卡的倒计时不再用 `core/mock` 的 `mockNow()`；改为 `core/time`：`now()` 是唯一读系统时间的入口，`useNow(ticking)` 供渲染期使用（共用一个秒表，满足 React 纯度规则）。测试用 `src/test/clock.ts` 把 `Date.now` 锚定到夹具日期，Mock 网关的 `clockOffsetMs` 归零，界面与 Mock 网关共用同一个"现在"。
- 网关装配去掉写死的 `mode: "mock"`（预测早已是真实平台，DEX 仍为 Mock 且尚未接入）。
- 护栏：`features/predict/mock-boundary.spec.ts` 静态扫描预测目录、首页与网关装配，禁止引用 Mock 网关、夹具、`core/mock` 与 `mockNow()/simulate()`；ESLint 对 `src/features/predict/**` 生产文件加 `no-restricted-imports`。`MockPredictGateway` 与 `fixtures/` 保留，仅供测试基座。
- 不在本次范围：DEX 继续用 Mock（未接入）；钱包估值的夹具价格表；生产租户预测链为 `op-sepolia` 测试网属产品配置。
