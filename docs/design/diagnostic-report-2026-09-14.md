...
## 13. 实施记录与偏离（2026-09-16）

### 13.1 落点

| 阶段                               | 仓库               | 提交                  |
| ---------------------------------- | ------------------ | --------------------- |
| A 密钥隔离闸 + 日志缓冲            | RN-App             | `05db618`             |
| B 六处日志来源                     | RN-App             | `0e90886`             |
| 共享脱敏向量                       | RN-App / RN-Server | `8a2b294` / `f0e1aa5` |
| C 接收、存储、管理端接口、迁移 v46 | RN-Server          | `f0e1aa5`             |
| 契约同步                           | RN-App             | `36d2ea6`             |
| D 上报页与版本信息                 | RN-App             | `f042666`             |
| E 崩溃快照与自动上报               | RN-App             | `c4dfcdf`             |
| F 管理端                           | RN-Admin           | `70124f3`             |
| 文案种子同步                       | RN-Server          | `6850d7a`             |
| 自审修正                           | 三仓               | 见各仓最后一个提交    |

三个仓库均在 `feat/diagnostic-report` 分支，**均未推送**。
...
| §        | 设计                                                      | 实现                                                                                      | 原因                                                                                                                                                           |
| -------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.2      | 引入只收错误码的 `WalletError`                            | 不引入；只加防回退 lint 闸                                                                | 核查后 keygen/vault 全部抛出点都没有拼运行时值（唯一的模板字符串是 `${MIN_PASSPHRASE_LENGTH}` 常量）。现状无缺陷，再叠一个错误类是纯改动面                     |
| 4.2      | 现场带 `networkType`                                      | 不带                                                                                      | 项目没有在用的网络状态读取；为一个可选字段接 netinfo 并处理其测试 mock 不值得。服务端字段保留，可选                                                            |
| 4.5      | 快照存错误消息与 componentStack                           | 只存错误类名、栈顶帧**名字**、崩溃时版本、日志尾巴                                        | 崩溃那条日志已在尾巴里且过了出口扫描，不再单独存一份未经扫描的；帧带文件路径（开发构建含用户名）                                                               |
| 4.5      | 根错误边界 / 全局异常都留快照                             | 只有渲染崩溃与**致命**全局错误留                                                          | 非致命的未捕获异常用户看不到崩溃，给每一个都留快照等于下次启动为小异常各自动上报一次                                                                           |
| 4.6 闸 1 | 远程开关 = `crashAutoReport`                              | = `diagnosticsEnabled && crashAutoReport`                                                 | 总闸关着时服务端回 404，快照会被当作"服务端拒收"丢弃                                                                                                           |
| 4.6 闸 5 | 熔断后丢弃快照                                            | 刚检测到循环：熔断 + 丢弃；**已在熔断中**的新快照：留给人工                               | 熔断的约定是"直到用户手动操作一次"，手动上报正是那一次；丢掉就连这条路也没了                                                                                   |
| 4.6 闸 5 | "连续 3 次启动都在 60 秒内崩溃"                           | 同义实现：启动记录带 `crashed` 标记，要求连续 3 次**崩了且**没活过 60 秒                  | 第一版只看"没活过 60 秒"，自审发现会把"打开看一眼余额就关"连着三次误判成崩溃循环、永久熔断，已修正并补回归用例                                                 |
| 4.6      | 上报失败                                                  | 服务端拒收（429/404）丢弃快照；网络、凭证类留到下次启动                                   | 设计未细分                                                                                                                                                     |
| 5.1      | 请求体带 `log: {entryCount, byteSize}` 声明，供第二步比对 | 不带                                                                                      | 客户端声明的值不可信，第二步服务端按实际解析与落盘大小记账，比对没有增量价值                                                                                   |
| 5.1      | 伪造身份字段"忽略"                                        | 直接 400                                                                                  | `decodeLimited` 拒绝未知字段，比忽略更严格                                                                                                                     |
| 5.6      | 表里有 `wallet_address`、`device_client_id`               | 删去两列，读时关联                                                                        | RN-Server AGENTS.md「一个事实只有一个存放处」。版本/渠道/运行中修订号保留：它们是上报时刻的快照，与 `app_installations` 最新状态不是同一个事实                 |
| 5.6      | 列表索引与预算索引分开                                    | 合并为覆盖索引 `ix_diag_tenant_time(tenant_id, created_at, byte_size)`；另加 `updated_at` | 一个索引同时服务排序与求和                                                                                                                                     |
| 5.7      | 批量删除按筛选条件                                        | 只收显式 ID（≤200）；部分失败返回 207                                                     | 无保留期下，筛选条件写错一次就是整个租户的报告找不回                                                                                                           |
| 5.8      | `GET /v1/admin/installations/:id/diagnostics`             | 不加；用列表接口的 `installationId` 筛选                                                  | 同一数据两个接口                                                                                                                                               |
| 5.8      | —                                                         | `GET /v1/admin/diagnostics/reports/:id` 保留但管理端暂未调用                              | 详情面板直接用列表项；接口在 OpenAPI 中，供自动化通道与后续深链                                                                                                |
| 6.1      | release-management 插件导航加第三项                       | 「运营数据」工作区加第四个一级 Tab                                                        | 运营数据已把设备/账号/通知收成 Tab（插件里有"旧页面保留可直达"注释）。已按 RN-Admin 工程标准 §12 同步修改标准里的 IA 表                                        |
| 6.2      | 账号列可跳钱包用户详情                                    | 只显示地址；设备列可打开安装实例详情                                                      | 钱包用户详情在另一个 Tab，没有现成的深链                                                                                                                       |
| 6.3      | 下载原文件                                                | 带凭据 fetch → blob → objectURL                                                           | 管理端会话 cookie `SameSite=Strict`，控制台与接口不同站时 `<a href>` 带不上                                                                                    |
| 7.1      | 平台显示 "Android 14"                                     | "Android API 35" / "iOS 17.2"                                                             | `Platform.Version` 在 Android 上是 API 级别，写成 "Android 35" 是误导                                                                                          |
| 7.2      | 点「上报问题」弹一个面板                                  | 独立页面                                                                                  | 要放输入框和可能几百行的预览；项目里没有底部面板内输入框的先例，键盘处理没有现成方案                                                                           |
| 7.4      | —                                                         | 根错误边界去掉屏幕上的"诊断 ID"                                                           | 它只在本机日志里，客服查不到；有了参考号只会让用户不知道该报哪个号                                                                                             |
| 8        | —                                                         | 管理端全部文案走 `text()`，中英文同步                                                     | RN-Admin 工程标准 §7                                                                                                                                           |
| 9        | 端到端：预发构建种 canary 走完整上报                      | **未做**                                                                                  | 没有预发构建与真机。由三处单测替代：App 出口（根错误边界 canary 用例断言上报内容无助记词）、服务端二次脱敏（落盘内容断言）、崩溃快照落盘（断言磁盘上无助记词） |
| —        | OpenAPI "生成文件禁止手改"                                | 手工编辑                                                                                  | 仓库里没有生成器，近期提交一直手工维护；沿用现状                                                                                                               |
...
- **崩溃循环误判**（见上表 4.6 闸 5）。
- **lint 闸把全局禁令顶掉**：flat config 同名规则整体覆盖，给 vault/keygen 单设 `no-restricted-syntax` 的第一版让 `console.log(phrase)` 在 vault 目录里能过。改为共享 `restrictedSyntax` 常量。
- **组件栈正则只认旧格式**：单测过是因为喂的是手写的 React 18 格式；React 19 是 `at X (…)`。
- **Proxy 以谁为 this 的用例抓不到 bug**：Babel 宽松模式下私有字段是普通属性，改坏照过；换成"内部调用不重复记录"这个真实后果。
- **配额"两类分开计"只验了一个方向**：变异测试发现合并计数照样绿，补反方向。
- **管理端 features schema 会剥掉 `crashAutoReport`**：保存配置是整份 PATCH，不补这一项的话管理员改任何配置都会把它悄悄关掉。
- **列表 `hasMore` 恒为 false**、**删除与审计不在同一事务**、**下载链接未挂进 DOM**（Firefox 不触发）。
- **上报预览里又出现了那串 update id 却标着"JS 与资源热更新"**——正是本需求的起因，改为明确标注"运行中的热更新（update id）"。
- 死代码：`reportAppOf`、`toNdjsonLine`、Go 侧只被测试使用的 `findSecretKinds`、管理端未调用的 `adminApi.diagnosticReport` 与 `DiagnosticReportList`；5 个只在文件内使用的导出改为不导出；`VersionInfoRow`、根错误边界的诊断文本拼接与剪贴板依赖随功能替换删除。

守安全不变量的逻辑都做了变异测试（把实现改坏确认用例变红）：RN-Server 11 条（移动端接口 7、管理端接口 4）、App 崩溃自动上报闸门 8 条、钱包网关 1 条；lint 闸 7 条用探针文件验证会触发。RN-Admin 前端没有做变异测试。

### 13.4 未验证

- 真机：版本信息面板、上报页、键盘、崩溃快照在致命错误下能否写完（设计 §4.5 已注明尽力而为）
- 预发/线上部署与迁移 v46 在生产库上的执行（只在本机 `rn-test-mysql` 跑过）
- 管理端真实浏览器（只有 jsdom 测试与生产构建）
- 端到端 canary（见 §13.2 第 9 条）
