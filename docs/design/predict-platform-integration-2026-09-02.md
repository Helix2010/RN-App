...
| 争议        | **更正（2026-09-06）**：网页版有用户争议流程（`ResolutionProgress.tsx` `RaiseDisputeModal`）：`POST {gamma}/disputes/evidence` 登记证据（无鉴权，150–1000 字、≤5 条 https 链接、条件须 `proposed`）→ EOA 对 LightOracle `approve` + `disputePrice(适配器, identifier, requestTimestamp, ancillaryData)`，押金 USDW 从 EOA 扣；`crypto_periodic` 市场无争议窗口。事实表与 App 方案见 `review-2026-09-05.md` §4.3                                                                                                                                                                         | grep `apps/user-dapp/src`                                                                                                        |
...
| WS 行情         | `wss://clob-ws.{domain}/ws/market`，订阅帧 `{assets_ids, level}`（1 = quote，2 = depth）；事件 `event_type: book \；实测（2026-09-03 用 node `ws`直连 prax1s）：初始 dump`book`的`timestamp` 是 ISO 串、`hash`为空，之后会来空的`price_change`（`price_changes: []`，`market: ""`）与 `last_trade_price {asset_id, data:{price}}`；文本 `PING`回`PONG`。展示价只认 0 < p < 1（`adapters.ts:566-567`、`orderbookPricing.ts` isTradablePrice，gamma 用 0 表示没数据） | price_change \| tick_size_change`，`price_changes[{asset_id, price, size, side, best_bid, best_ask}]` | `wsservice/market_channel.go:31-90`；§2.7 |
...
**我们的映射决定（非平台事实）**：`Market.id` 用 `conditionId`（clob 的 `market`、data 的 `conditionId` 都以它为键）；`PredictEvent.id` 用 gamma 事件 id、`slug` 用事件 slug；价格换成整数分（`round(p×100)`）；`yesTokenId / noTokenId` 取 `clobTokenIds[0] / [1]`；余额、成本、盈亏用 6 位 USDW 的 `Money`（预测账户内的抵押品单位；账户余额同为 USDW，同币种才能比较——2026-09-03 联调改正，此前标成 USDC 导致下单页崩溃）；`submitDispute` 已按 `review-2026-09-05.md` §4.3 实现（证据登记 → 押金核对 → 授权 → EOA 调 LightOracle `disputePrice`，2026-09-06；见 `docs/changes/2026-09-06-feature-predict-dispute.md`）；多结果事件下每个市场一行、标题取 `groupItemTitle`。
...
**平台侧隐患补充（争议，2026-09-06）**：`POST /disputes/evidence` 无鉴权，任何人可替任意 `disputer` 地址登记证据意向（只在链上 `PriceDisputed` 匹配时才 commit，风险可接受但需知晓）；押金币种是 USDW，而用户的 USDW 在 Safe 里，争议却只能从 EOA 发（relayer 白名单无 LightOracle）——若平台把 LightOracle 加入 `security.contractWhitelist`，App 可改走 Safe 免 gas；`adjudication` 不返回 `bond`，客户端必须读链 `getRequest`。


实现状态（2026-09-02）：阶段 1–5（服务端下发、管理端、App 平台客户端、账户网关、启用 / 划转界面）已落地，见 `docs/changes/2026-09-02-feature-predict-account-real.md`。阶段 6 读侧已落地（`HttpPredictGateway`：标签、事件列表 / 详情、订单簿、价格历史、费率、结算状态、持仓 / 已平仓、活动、盈亏、排行榜、我的挂单、撤单；事实见 §2.9），生产接线已从 `MockPredictGateway` 切到它。写侧也已落地：下单（EIP-712 Order 签名，maker = Safe / signer = EOA，金额换算逐行移植 `orderAmounts.ts`，市价 = FAK 取对手盘最优价，`POST /order` 带 L2 头）、领取（同 conditionId 合并一条 `redeemPositions`，链上 ERC1155 余额为准，MultiSend 经 relayer）、拆合（直接 SafeTx 调 CTF / adapter，operation 0）。WS 推送也已接（`core/predict-platform/market-ws.ts`：`wss://clob-ws.{domain}/ws/market`，首帧 / 增量订阅帧、10 秒文本 PING、1s → 30s 退避重连并重发订阅，`book` / `price_change` 映射为 `subscribeMarkets` 事件；事实见 §2.7 / §2.9）。模拟器对 prax1s 的读侧联调已过（真实标签 / 事件 / 详情 / 订单簿），由此修了分类标签、无报价占位、`useMarketStream` 消费 WS、首页标签四处，见变更说明。2026-09-03 模拟器对 prax1s 的端到端联调全部通过：建钱包 → 登录 → 四步启用 → 转入（approve + wrap）→ 市价成交 / 限价挂撤 → 持仓 / 活动核对 → 两阶段取回到钱包；过程中修正的问题见变更说明。2026-09-03 模拟器对 prax1s 的端到端已过：建钱包 → 登录 → 四步启用 → 转入（approve + wrap）→ 市价买 × 3 → 限价挂单 + 撤单 → 市价卖 → 拆分 / 合并 → 两阶段取回（initiate → 60 秒 → claim，USDC 回钱包）。剩余：争议提交已于 2026-09-06 实现（`review-2026-09-05.md` §4.3），dev 端到端待一个处于 `proposed` 的非 crypto_periodic 市场；领取（redeem）要等一个已结算市场；dev 水龙头阈值疑点待平台侧核对；发布：自 1.2.7 基线（`22b3cff`）以来 `package.json` / `app.config.ts` / 原生插件无改动，本轮全是 TS / 文案，按 `docs/SAAS_TENANT_BUILD_RUNBOOK.md` §5 可走 OTA（runtimeVersion `embedded` 对应 1.2.7）让现网用户直接拿到真实预测市场，无需新 APK；是否发布由运营决定。2026-09-03 下午全面评审修正已合入（下单规则、手续费公式、应答只信份数、拒单原因透出、结算 / 可领判定、WS 守卫，见变更说明）。修正后的下单面板已在模拟器对 prax1s 回归（实时价、市价 / 限价下限与 tick 提示、限价挂撤、1.01 USDW 市价成交 3.36 份）；WS 新增代币改发 `type:"market"` 帧以拿初始簿（`market_channel.go`：该帧只增不清订阅）。当前遗留：OTA / 新包发布由运营拍板；redeem 待已结算市场；争议提交已实现、待 dev 端到端；平台侧四项待平台方核对（dev 水龙头阈值、`POST /order` 成交额、EIP-712 `app_name` 不在 public-info、CLOB secret 编码）。
...
全面评审（2026-09-03 下午，对照 `match_dispatcher.go:370-400`、`plan.go:108-196`、`positions.go:426-455`、`matcher.go`、`market_channel.go:376-432`）后修掉：下单规则（限价份数 ≥ `min_order_size`、市价买 ≥ 1 USDC、限价 tick 对齐，`ORDER_PRICE_NOT_ALIGNED`）、无报价不编 50¢、手续费 = bps × min(p, 1 − p) × 份数 / 1e4（买入扣份额、卖出扣回款）、`POST /order` 应答只信份数、拒单 `errorMsg` 透出、结算价与可领判定、挂单方向按 token id、WS 旧连接 close 守卫、clob 余额强刷改 await。`POST /order` 应答 `status` 取值（`match_dispatcher.go:1922-1930`）：`matched` / `live` / `canceled`（FAK 零成交，金额为 0）/ `delayed`；市价买还要满足份数对齐 0.01 后 makerAmount ≥ 1 USDC，App 按当前对手价反推最小预算（`OrderPreview.minAmount`）。**平台侧隐患补充**：`POST /order` 的 `takingAmount` / `makingAmount` 在 `matcher.go` 里都写成 fillAmount，客户端拿不到成交额（网页版同样只显示份数）；`/book` 的 `tick_size` 是 nullish 字段，缺失时 App 不做本地 tick 预判，由平台校验。
