...
- 主网的值平台还没定。如果是几小时或几天，App 就必须有"待领取"列表和到期推送，用户不可能盯着倒计时。
...
| 问题                 | 决定                                                                                                                    |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 先接哪条链           | **先调通测试网（OP Sepolia，dev 环境 predict.prax1s.xyz）**；主网（Monad 143）后续再加链，届时补 `PROTOCOL`、端点、目录 |
...
| 主网 `unwrapDelay`   | 主网延后，暂不追；实现按任意延迟设计，dev 按 60 秒联调                                                                  |

## 7. 待办（不阻塞开工）

1. 让平台把 `USDC_UNDERLYING` 加进 relayer 白名单（dev 与将来的主网各一次），否则转出第二阶段必败。
2. 主网接入前向平台确认 `unwrapDelay` 与 `minUnwrapUsdw` 的主网值，决定要不要做到期推送。
