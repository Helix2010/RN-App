...
- 接口 / 页面：账号列表带最近活跃设备摘要；总览"当前登录账号"按账号数统计；设备列表"当前账号"列显示会话最近使用时间；增加 APK 版本筛选；平台视图增加按设备归并 ID 查询（RN-Server `d7c5851`、RN-Admin `7b3c2d2`）。
- 数据库集成测试（RN-Server `internal/api/db_integration_test.go`，6 个用例，MySQL 8 容器实跑通过）：会话替代与登录历史累加、租户级封禁结束会话并审计、心跳解析运行修订号（关联不上存 NULL、内置包清空）、租户接口不泄露归并 ID 且派生当前账号与对账差异、平台级跨租户归并与审计、平台级封禁结束所有租户会话。
- 模拟器场景补跑（两台模拟器，账号 B = 0xbf53…7Df4）：场景 2 同一设备切换账号 A→B，管理端当前账号从 A 变 B，A 的会话结束原因 logout，账号历史两行；场景 3 同一账号在两台设备登录，账号详情两组设备均"当前"；场景 4 撤销其中一台的安装实例，App 回未登录态，会话结束原因 admin，该设备标 revoked；场景 6 设备归并：平台设备查询显示同一台模拟器历次重装的 4 个安装实例归到一组。
- 未覆盖：同设备两条有效会话被替代（App 切换账号时总是先登出，线上触发不到），由数据库集成测试覆盖；第二个租户的真实登录。
