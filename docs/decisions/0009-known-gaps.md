...
## 预测市场覆盖补全后的已知边界（2026-09-08）

- 周期市场（`/series`）：系列卡与系列页（当期窗口、参考价、倒计时、历史窗口、价格 / 概率 / K 线图与实时价）；交易复用事件详情，K 线不做 tick 合并（20 秒重拉）。
- 持仓状态由 data-service 的 `redeemable / marketSettled` 推出，不带裁决阶段，所以持仓不会显示"已取消"；取消只体现在事件 / 结算页。
- 流动性排序是当前页本地排序（网页版同样），分页各自排。
- 首页策展的"按分类分区"未做；本地"高概率 / 今日热门"榜只在默认排序下显示。
