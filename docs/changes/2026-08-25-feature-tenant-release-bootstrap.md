...
- 当前行为或复现：bootstrap 请求曾依赖 tenant 参数，多租户服务上线后存在错误租户配置和本地缓存串用风险。
...
1. Given 构建配置包含 API 域名与 application id，When App 请求 bootstrap，Then 不发送 tenant query，由服务端根据请求域名解析租户并返回该租户策略。
2. Given 同一设备先后运行不同租户构建，When 远端请求失败读取缓存，Then 缓存 key 按 API 域名、application id 和 locale 隔离，不读取其他租户配置。
3. Given staging/production 构建缺少 API 域名或 application id，When 生成 Expo 配置，Then 构建提前失败，不产生身份不明确的包。
...
- API/OpenAPI：`GET /v1/mobile/bootstrap` 以请求域名作为租户上下文；服务端不再读取 tenant query。
...
- 回滚：当前为开发阶段，回退时重新构建目标版本并重新初始化开发库。
