...
- 下发变化后要重读的查询补齐：此前配置变化只失效连接器与余额，资产总览（首页金额）和最近转出没跟着重读，管理端加链 / 上币后首页会停在改动之前。
- 转出成功后失效的是 `["wallet-transfers"]`——没有任何查询用这个键；改为失效 `["wallet-recent-recipients"]`，刚转过的地址立刻出现在"最近转出"里。

## 配置改动何时生效（管理端 → App）

| 环节          | 时机                                                                                                               |
| ------------- | ------------------------------------------------------------------------------------------------------------------ |
| 管理端保存    | 立即写库，下一次 bootstrap 请求就能读到；服务端没有缓存层                                                          |
| App 重新拉取  | 冷启动（启动门禁等它到达才进业务页）、每次回到前台、下发的刷新间隔到点（当前租户 21600 秒 = 6 小时）、收到推送信号 |
| 应用到运行时  | 数据到达时在 `bootstrapQueryFn` 里 `applyDeliveredWalletConfig`，链集合 / 端点 / 代币目录整体换新                  |
| 界面重读      | 钱包段引用变化触发失效：连接器、余额、资产总览、最近转出                                                           |
| RPC 端点      | 链客户端持有 `rpcUrlsFor(chain)` 闭包，下一次请求就走新端点；不重建客户端，也不用重启                              |
| WalletConnect | projectId / 链集合 / onchainSends 变了丢弃已建客户端，下次连接用新的；已批准的会话不会重新协商                     |
