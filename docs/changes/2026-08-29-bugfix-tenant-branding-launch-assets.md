...
- 当前行为或复现：Bootstrap 返回租户 Logo 和启动背景，但启动门禁没有等待远程 Branding hydration，可能在首次启动直接进入首页或继续显示内置 BrandMark；生产 channel 旧 OTA 还可能覆盖本地 Bundle。
...
- loading / empty / content：远程配置成功后，在既有开屏时长内等待一次 Branding hydration，优先显示当前 Bootstrap 的租户资源。
...
- iOS / Android：运行 `pnpm check`；Android 模拟器用干净 Smoke APK 验证远程 Bundle 加载和启动门禁，已确认线上 Logo/背景接口 HTTP 200；旧 production OTA 覆盖行为已通过 logcat 证实，发布新 OTA 后需再做真机首次启动与本地缓存复测。
- S-02 设置页已按设计稿拆分通用、通知、交易偏好、安全和关于分组；语言/外观通过独立二级页面进入。
