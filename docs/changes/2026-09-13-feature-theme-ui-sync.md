...
- 品牌与外观新增基础配置、主题、业务模块、功能开关、升级策略五个 Tab。
- 这些 Tab 复用配置中心现有的校验、版本控制、修改原因、审计和保存接口；主题不再维护第二套编辑器。
- 配置中心从侧边导航移除，旧 `config` 页面仍保留直接访问兼容。
- 品牌与外观初始加载、应用身份加载和嵌入式配置加载均使用 Skeleton。
- 后续信息架构收敛：配置中心不再出现在侧边导航，品牌与外观新增基础配置、业务模块、功能开关、升级策略 Tab，并复用配置中心原有保存逻辑；旧配置中心页面仍保留直接访问兼容。
