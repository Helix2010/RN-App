...
- Android Development Build（`rn_smoke`，2026-08-30 15:25–15:31）：
  ...
  - 追加验证（2026-08-31 02:42，英文 / 浅色主题）：限价 +100 份 @ 64.5¢（盘口提示 65 / 66，GTC/GTD 切换）→ "Order placed"，挂单 Tab 3 → 撤单 → "Order cancelled"、可用余额回到 1,240.50；持仓行"卖出" → 卖出态（持有 300 份，50% → 150）→ "Filled 150 shares @ 60¢"，可用 1,330.32。✅ `limit-order-cancel.png` / `sell-flow.png`
  - 游客态：预测列表无余额 chip → 点专场"买 Yes" → 登录 sheet 副标题"登录后继续买入 Yes" → MetaMask 签名 → 自动进入事件详情并弹出下单面板（法国 · Yes 24¢）。✅ `guest-intent-replay.png`。修复：意图回放曾在渲染期间更新 store（React 报错），改到 effect。
  - 修复：世界杯各市场 `endsAt` 误为 2026-07-19（已过）导致"已结算"，改为与事件一致 2027-07-19；结算推导不再落盘。
  - 拆分 / 合并 sheet（持仓汇总卡与预测账户详情入口）：拆分 / 合并段控、市场段控（仅未截止二元市场）、数量（可用 / 可合并 上限）、说明；拆分 100 USDC → toast → 三段进度到已确认，持仓 5 → 6、可用 1,330.32 → 1,230.32。✅ `split-flow.png`（2026-08-31 02:51）
  - 未在设备验证：争议提交 sheet（需一个"结果已提交且在争议期内"的市场可从列表进入，当前列表过滤已截止事件）、多结果事件段控切换、iOS。
- 回滚：回退本分支提交；无数据迁移。
