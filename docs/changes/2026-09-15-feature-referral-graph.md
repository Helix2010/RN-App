...
**这两条后来都被推翻了**（用户要求按代码复核，复核结论如下）：

3. ~~服务端把文案键名小写化，所以 App 侧 22 个驼峰键运营在控制台里改不了。~~
   **这条是错的，撤回。** 复核代码：App 的 `translateMessage` 查找前会过
   `normalizeMessageKey`（小写），`normalizeMessages` 把服务端下发的键小写化，
   `builtinMessages` 也过同一个函数——**两端一致小写，驼峰键可以被服务端覆盖**。
   实测钉死：把 `referral.myCode` 在控制台改成探针值并发布，模拟器上邀请页显示的
   就是探针文案。另外「邀请是第一个大量用驼峰键的模块」也是错的——全仓 1241 个键
   里 550 个含大写（`predict` 150、`update` 71、`send` 42），邀请的 22 个排第 8。
   源码里用驼峰只是可读性惯例，不影响可翻译性。
4. ~~装新 APK 后的第一次启动可能跑旧 OTA 包。~~ **这条复现不出来，降级为"观察到一次
   异常、机制未确认"，不要当成已知缺陷写进发版说明。**

   观察到的现象是真的：在一台原先装着 1.3.15、且有 OTA 待应用的模拟器上覆盖安装
   1.3.16 之后，第一次启动显示的是旧界面（DEX 版首页、「邀请返佣」且点击无反应），
   第二次启动恢复正常。但我清了那台设备的数据，`updates.db` 被重建，证据没了。

   **专门构建 1.3.15/45 复现失败**：干净装上它、等它自动检查 OTA，`logcat` 里
   expo-updates 报的是 `CheckCompleteUnavailable`——**没有可用更新**。原因是
   expo-updates 不会回退到比内置包发布更早的更新，而刚构建的 APK 内置包总是最新的。

   代码侧有三道各自独立的闸，任一成立就不会发生：
   - `app.config.ts` 的 `runtimeVersion: appVersion`，每个原生版本自己一个
     runtimeVersion，1.3.15 的更新不是 1.3.16 二进制的候选；
   - expo-updates 不启动比内置包更早发布的更新（上面实测到的那条）；
   - `update-service.ts` 的 `isManifestCompatibleWithApp` 在检查与下载两处
     比对 version + buildNumber。

   所以当时看到的更可能是**缓存的 bootstrap**（`ttlSeconds` 6 小时，首页模块布局
   来自配置而不是 JS 包）叠加安装前就在屏上的那个更新弹窗，不是"旧 OTA 被启动"。
   要确认得留着现场看 `updates.db` 与 `Updates.isEmbeddedLaunch`，下次遇到别先清数据。

## 界面与本仓既有页面对齐（2026-09-15，实测反馈「好丑、交互不契合」后返工）

装到模拟器上截图逐项比对，确实和本仓其它功能页差得远。都不是主观取舍，是漏用了
既有范式，逐条改掉：

| 问题                               | 实际表现                                                                 | 改成                                                                                                                       |
| ---------------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `ScreenHeader` 没包安全区          | 标题顶进状态栏、返回箭头贴着屏幕左边                                     | 按 records-screen 用 `Content paddingTop={insets.top + 8}` 包一层，并补 `backLabel`                                        |
| 一行挤三个次级按钮                 | 文案被截成「Copy c...」「Copy li...」                                    | 分享升为全宽主按钮（它才是这页的目的），复制码/复制链接两个带图标的次级按钮一行——本仓的行范式就是每行两个（receive-sheet） |
| 二维码没有白底托                   | 主题色的底配主题色的码，深色模式下不少相机扫不出来，观感也和收款页不一致 | 照 receive-sheet 套白色圆角托，尺寸 160 → 196                                                                              |
| 下级列表是两列裸文字               | 没有头像、没有分隔线、没有上下内边距，和本仓任何列表都不像               | 照 records-screen 的行范式：36px 圆形图标 + 两行左块（14px 标题 / 11px 副标题）+ 下边框                                    |
| 邀请人是一句揉在一起的裸文字       | 邀请码和绑定时间挤在一个句子里                                           | 拆成两个标准 `DetailRow`（13px 灰标签 + 加粗值），不可更改那句降为 11px 注脚                                               |
| 时间是 `toLocaleString()` 原样输出 | `9/16/2026, 7:32:44 AM`，和其它页的格式不是一套                          | 统一走 `formatDateTime(x, locale)` → `Sep 16, 07:32`                                                                       |
| 没有下拉刷新                       | 其它数据页都有，这页下拉没反应                                           | `PageScroll` 接上 `refresh`，同时重拉概览与下级列表                                                                        |

核对过**不是**问题、不动的：禁用态按钮的观感是设计系统统一的
`disabledStyle: { opacity: 0.45 }`，主次按钮一致；二维码 196px 与白托也与收款页一致。
